...
SELECT ENAME , REVIEW1(SAL) , REVIEW1(10)
FROM EMP;
...
BEGIN
    ...
    IF P_COUNT = 0 THEN 
        dbms_output.PUT_LINE('해당교수번호가 존재하지 않습니다.');
    ELSIF P_COUNT = 1 THEN
        dbms_output.PUT_LINE('정상적으로 수정되었습니다..');
    ELSE 
        dbms_output.PUT_LINE('2개이상의 교수 보너스가 수정되엇습니다.');
    END IF;
    COMMIT;
END;
/
...
EXECUTE REVIEW2(1002,10);

SELECT * FROM STUDENT;


--20153075 학번 학생의 키가 1증가
-- 없는 학번을 입력후 실행하면 '해당 학생이 없습니다.' 출력
...
(   
    ...
    P_INCREASE IN STUDENT.STU_HEIGHT%TYPE
)
...
BEGIN
    UPDATE STUDENT 
    SET  STU_HEIGHT = STU_HEIGHT +P_INCREASE
    WHERE STU_NO = P_STUNO;
    P_CNT := SQL%ROWCOUNT;
    IF P_CNT = 0 THEN
    dbms_output.put_line('해당학생이없습니다.');
    ELSIF P_CNT =1 THEN
    dbms_output.PUT_LINE('수정되었습니다.');
    ELSE 
    dbms_output.PUT_LINE('2명의 학생이 수정되었습니다.');
    END IF;
END;
/
